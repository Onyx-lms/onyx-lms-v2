 ...
 *
 * Over HTTP by the catch-all route, for the browser -- and IN PROCESS by
 * `apps/web/src/lib/onyx-session.ts` and its console twin, for every server
 * component that needs data to render.
 *
 * The second is why this file exists. A page used to fetch its own API at its
 * own public hostname: out through the CDN, a TLS handshake, a second
 * serverless invocation, and back. Measured, that cost 250-350ms per call
 * before a single row was read -- and a page makes three or four calls, so
 * most of a second on every navigation was the application phoning itself.
 * Calling the handler directly removes all of it and changes nothing about
 * what the handler does: the same route table, the same guards, the same
 * envelope, the same audit.
 *
 * `cookies()` still works, because a server component runs inside the same
 * request. `after()` still works, for the same reason.
 */
